...
public class EntityMappingImpl implements EntityMapping
{
	...
	/**
	 * Initializes this {@link EntityMapping} based on the owl:sameAs
	 * statements in the given RDF graph; assumes that the subject of
	 * every owl:sameAs statement is a local URI and the object is
	 * the corresponding global URI.
	 */
	public EntityMappingImpl( final Graph mappingDescription ) {
		this( parseMappingDescriptionIntoG2LMap(mappingDescription) );
	}
	...
	protected static Map<Node, Set<Node>> parseMappingDescriptionIntoG2LMap( final Graph mappingDescription ) {
		final Map<Node, Set<Node>> g2lMap = new HashMap<>();

		// populate g2lMap based on the assumption that the subject of
		// every owl:sameAs statement in the given graph is a local URI
		// and the object is the corresponding global URI
