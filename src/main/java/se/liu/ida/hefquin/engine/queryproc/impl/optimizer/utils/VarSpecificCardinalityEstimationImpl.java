...
public class VarSpecificCardinalityEstimationImpl implements VarSpecificCardinalityEstimation
{
			...
			final Var v )
	{
		...
		if ( rootOp instanceof LogicalOpRequest ) {
			...
			final PhysicalPlan reqTP = formRequestPlan( (LogicalOpTPAdd) rootOp );
			return _initiateJoinCardinalityEstimation( plan.getSubPlan(0), reqTP, v );
		}
		else if ( rootOp instanceof LogicalOpBGPAdd ) {
			final PhysicalPlan reqBGP = formRequestPlan( (LogicalOpBGPAdd) rootOp );
	...
	protected static PhysicalPlan formRequestPlan( final LogicalOpTPAdd lop ) {
	...
	protected static PhysicalPlan formRequestPlan( final LogicalOpBGPAdd lop ) {
		final FederationMember fm = lop.getFederationMember();
	...
	protected static PhysicalPlan formRequestPlan( final SPARQLGraphPattern p,
	...
	protected static PhysicalPlan createRequestPlan( final FederationMember fm,
		...
		final PhysicalOpRequest<?,?> pop = new PhysicalOpRequest<>(lop);
