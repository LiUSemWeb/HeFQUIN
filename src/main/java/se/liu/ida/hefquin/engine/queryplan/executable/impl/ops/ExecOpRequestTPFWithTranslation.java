...
public abstract class ExecOpRequestTPFWithTranslation<MemberType extends FederationMember> extends ExecOpGenericTriplePatternRequestWithTPF<MemberType>{
	...
	public TPFResponse performPageRequest( final TPFRequest req, final FederationAccessManager fedAccessMgr ) throws FederationAccessException {
	...
	protected Set<Triple> handleTriplePattern(final TriplePattern tp, final FederationAccessManager fedAccessMgr) throws FederationAccessException{
		...
		if (fm instanceof BRTPFServer) {
			res = FederationAccessUtils.performRequest(fedAccessMgr, newReq, (BRTPFServer) fm);
		} else if (fm instanceof TPFServer) {
			res = FederationAccessUtils.performRequest(fedAccessMgr, newReq, (TPFServer) fm);
		} else {
			throw new FederationAccessException(fm.toString(), req, fm);
		}
